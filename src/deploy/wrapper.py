    ...
    def predict(self, context, model_input):
        ...
        try:
            if isinstance(model_input, pd.DataFrame) and len(model_input) > 1:
                ...
                # --- CORREÇÃO DEFINITIVA: CAPTURA SEGURA DOS IDS ---
                # Criamos um mapa ordenado dos IDs originais (Strings) ANTES de qualquer transformação.
                # Isso blinda o código contra qualquer erro de encoding (-1.0) ou perda de metadados.
                store_ids_map = []
                for ts in target_series_list:
                    # O ID original está no índice das estáticas assim que sai do from_group_dataframe
                    raw_id = str(ts.static_covariates.index[0])
                    store_ids_map.append(raw_id)

                # --- CONSTRUÇÃO DE COVARIÁVEIS ---
                ...
                for i, ts_target in enumerate(target_series_list):
                    store_id = store_ids_map[i] # Usa o ID capturado via posição
            ...
            # --- PREDIÇÃO ---
            # O Darts garante que a ordem da saída é a mesma da entrada
            ...
            # --- MONTAGEM DO DATAFRAME FINAL (METODOLOGIA "MELT" / ZIP) ---
            final_df_list = []
            
            # Iteramos juntos: A predição e o ID original que salvamos lá no começo
            for pred_series, original_store_id in zip(pred_series_list, store_ids_map):
                
                # Inverte apenas o valor numérico (Venda)
                pred_inverse = self.pipeline.inverse_transform(pred_series, partial=True)
                df = pred_inverse.pd_dataframe()
                
                # Atribui o ID Original (String) explicitamente
